    ...
    def track_video(self,video:str, output:str, skip_frames:int=0, show_live:bool=False, count_objects:bool=False, verbose:int = 0):
        ...
        while True: # while video is running
            ...
            for track in self.tracker.tracks:  # update new findings AKA tracks
                ...
                area_2 = [(180,520),(331, 504),(356, 563),(163,583)]    # video points  ############################## this are the two polygons drawn TL, TR,BR,BL
                area_1 = [(127,684),(425, 646),(454, 702),(109,736)]    # video points 
                ...
                # print("vehicles_entering: ",vehicles_entering)
                if track.track_id in vehicles_entering:    # if vehicle from vehicle entering dictonary is passed through another polygon
                    ...
                    if result>0:  # if passed
                        ...
                        cv2.putText(frame, 'Reverse',(int(bbox[0]), int(bbox[1]-11)),0, 0.6, (255,255,255),1, lineType=cv2.LINE_AA)    
                ...
                for i,area in enumerate([area_1, area_2]):
                    ...
                    # print(frame.shape)          ###############
                cv2.polylines(frame, [np.array(area_1, np.int32)],True, (255, 0, 0),6)  #Red # this draws virtual polygons over road 
                cv2.polylines(frame, [np.array(area_2, np.int32)],True, (15,220,10),6) #Green


                # LANE 2 ###################################################################################-----------------------------------------------------------

                area_2 = [(331, 504),(475,499),(530,548),(356, 563)]    #old video # this are the two polygons drawn TL, TR,BR,BL 
                area_1 = [(425, 646),(670,625),(718,664),(454, 702)]
                ...
                result = cv2.pointPolygonTest(np.array(area_2, np.int32), (int(cx), int(cy)), False) # this validates car center passed polygon or not 

                if result>=0:  ################ if the polygon is passed 
                    # vehicles_entering2[track.track_id]=(cx,cy)
                    vehicles_entering2[track.track_id]=time.time() # store starting time with its id in dict.
                # print(vehicles_entering2)
                if track.track_id in vehicles_entering2:    # if vehicle from vehicle entering dictonary is passed through another polygon
                    result = cv2.pointPolygonTest(np.array(area_1, np.int32), (int(cx), int(cy)), False) # vehicle passsed through another poly testing
                    if result>0:  # if passed
                        elapsed_time = time.time() - vehicles_entering2[track.track_id] ## calculate total time taken

                        if track.track_id not in vehicles_elapsed_time2: # if time taken of id not in list store it with id
                            vehicles_elapsed_time2[track.track_id] = elapsed_time

                        if track.track_id in vehicles_elapsed_time2:
                            elapsed_time = vehicles_elapsed_time2[track.track_id] # assign value to each track id
                ...
                for i,area in enumerate([area_1, area_2]):
                        ...
                        # cv2.polylines(frame, [np.array(area, np.int32)],True, (15,220,10),6)    # this command prints polygon
                    # print(frame.shape)          ###############
                cv2.polylines(frame, [np.array(area_1, np.int32)],True, (15,220,10),6)   #Green # this draws virtual polygons over road 
                cv2.polylines(frame, [np.array(area_2, np.int32)],True, (255, 0, 0),6)      # Red
