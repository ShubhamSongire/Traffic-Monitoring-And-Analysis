...
class YOLOv7_DeepSORT:
    ...
    # sched.add_job(track_video, 'cron', minute=0)
    # sched.start()
    # referances http://localhost:8888/notebooks/Untitled-Copy1.ipynb, https://stackoverflow.com/questions/47741105/schedule-job-to-start-at-beginning-of-each-hour
        ...
        while True: # while video is running
            ...
            for track in self.tracker.tracks:  # update new findings AKA tracks
                ...
                if result>0:  ################ if the polygon is passed 
                    ...
                    if class_name=="bus":l_bus.add(track.track_id)
                    elif class_name=="car":l_car.add(track.track_id)
                    elif class_name=="bike":l_bike.add(track.track_id)
                    elif class_name=="truck":l_truck.add(track.track_id)
                    elif class_name=="auto":l_auto.add(track.track_id)
                    elif class_name=="tractor":l_tractor.add(track.track_id)
                    elif class_name=="traveller":l_traveller.add(track.track_id)
                    elif class_name=="LCV":l_LCV.add(track.track_id)
                ...
                # cv2.putText(frame, str(len(left_count)),(int(bbox[0]), int(bbox[1]-11)),0, 0.6, (255,255,255),1, lineType=cv2.LINE_AA)
                cv2.putText(frame, str(len(left_count)),(270,690),0, 0.6, (255,255,255),1, lineType=cv2.LINE_AA)
                cv2.rectangle(frame, (70,85), (720,100), color, -1)
                cv2.putText(frame, "LEFT TOTAL"+str(len(left_count))+' bus'+str(len(l_bus))+' car'+str(len(l_car))+' bike'+str(len(l_bike))+' truck'+str(len(l_truck))+
                ' auto'+str(len(l_auto))+' tractor'+str(len(l_tractor))+' traveller'+str(len(l_traveller))+' LCV'+str(len(l_LCV))
                ,(70,100),0, 0.6, (0,0,0),1, lineType=cv2.LINE_AA)
                ...
                cv2.polylines(frame, [np.array(area_1, np.int32)],True, (15,220,10),6)   # this draws virtual polygons over road 
